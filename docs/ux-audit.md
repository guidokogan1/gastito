...
14. ✅ Reemplazar `window.confirm` por un diálogo consistente con copy más claro.
...
42. ✅ Chips para filtros rápidos (hoy, semana, sin categoría).
43. ✅ Mostrar totales del filtro actual (gasto/ingreso neto).
44. ✅ Input de monto con preview ARS formateado.
...
52. 🧭 “Desactivar” como acción primaria y “Borrar” como secundaria (seguro). Se agregó copy preventivo en zona sensible; falta cambiar la acción primaria.
...
70. 🧭 Unificar “forms” repetidos en componentes (AuthCard, CrudCard). Se agregó `DangerZone`, `MoneyField`, `FinancialAmount` y `PasswordField`; falta completar CRUD editor reusable.
71. 🧭 E2E smoke básico (login → onboarding → movimiento) para evitar regresiones.

## Pase mobile/UI 2026-05-06

- ✅ Bottom nav mobile: 4 destinos principales + “Más” para Cuentas, Deudas y Gastos fijos.
- ✅ FAB persistente para crear movimiento desde cualquier pantalla privada.
- ✅ `Slideout`: focus trap, restore focus, scroll lock y ARIA con `labelledby/describedby`.
- ✅ Confirmaciones destructivas sin `window.confirm`.
- ✅ `MoneyField` con preview ARS y `FinancialAmount` con signos/tokens.
- ✅ Dashboard mensual con selector, comparación, proyección, gastos fijos y deudas.
- ✅ Filtros de movimientos persistidos localmente, quick chips y orden.
- ✅ Auth con show/hide password e indicador de fortaleza.
- 🧭 Pendiente fuerte: crear recursos desde el composer, e2e Playwright, visual regression y migración `category.type`.

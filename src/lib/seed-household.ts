import { DEFAULT_ACCOUNTS, DEFAULT_CATEGORIES, DEFAULT_INCOME_CATEGORIES, DEFAULT_PAYMENT_METHODS } from "@/lib/catalog";
...
export async function seedHousehold(db: SeedDb, householdId: string) {
  await db.category.createMany({
    data: [...DEFAULT_CATEGORIES, ...DEFAULT_INCOME_CATEGORIES].map((name) => ({
      householdId,
      name,
    })),
    skipDuplicates: true,
